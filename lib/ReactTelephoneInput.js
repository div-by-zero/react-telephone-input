...
var parseMask = function parseMask(_ref) {
    var dialCode = _ref.dialCode,
        format = _ref.format;
    return Array.from(dialCode).reduce(function (result, current) {
...
var ReactTelephoneInput = exports.ReactTelephoneInput = React.createClass({
    ...
    handleFlagDropdownClick: function handleFlagDropdownClick() {
        ...
        var country = find(this.props.onlyCountries, this.state.selectedCountry);
        this.setState({
    ...
    _mapPropsToState: function _mapPropsToState(props) {
        ...
        var selectedCountryGuess = this.state ? this.state.selectedCountry : this.guessSelectedCountry(inputNumber.replace(/\D/g, ''));

        var selectedCountryGuessIndex = findIndex(allCountries, selectedCountryGuess);
        var selectedCountryGuessFormat = selectedCountryGuess ? selectedCountryGuess.format : null;
        var formattedNumber = this.formatNumber(inputNumber.replace(/\D/g, ''), props.pattern || selectedCountryGuessFormat);
    ...
    render: function render() {
        ...
        var _state$selectedCountr = this.state.selectedCountry,
            format = _state$selectedCountr.format,
            dialCode = _state$selectedCountr.dialCode;

        var resolvedFormat = this.props.pattern || format;
        var mask = parseMask({ dialCode: dialCode, format: resolvedFormat });
        ...
        return React.createElement(
            ...
            React.createElement(
                'div',
                { ref: 'flagDropDownButton', className: flagViewClasses },
                React.createElement(
                    'div',
